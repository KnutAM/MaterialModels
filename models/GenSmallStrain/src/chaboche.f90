...
! Input:    props = E, v, Y0, Hiso, invYiso, Hk1, invYk1, Hk2, ...
...
subroutine checkinput(nprops, nstatv, props, mpar)
    ...
    if (error) then
        write(*,*) 'Invalid input to the Chaboche model (umat), it should be'
        write(*,*) 'nprops = 5+2*nback and'
    ...
    mpar(3:5) = props(3:5)
    do k1 = 1,nback
        mpar(7+3*(k1-1)) = props(6+2*(k1-1))
        mpar(8+3*(k1-1)) = props(7+2*(k1-1))
    enddo
