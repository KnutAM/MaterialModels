...
subroutine checkinput(nprops, nstatv, props, mpar)
    ...
    if (error) then
        write(*,*) 'Invalid input to the Delobelle rate dependent model (umat), it should be'
        write(*,*) 'nprops = 8+2*nback and'
    ...
    mpar(3:8) = props(3:8)
    do k1 = 1,nback
        mpar(9+3*(k1-1)) = props(9+2*(k1-1))
        mpar(10+3*(k1-1)) = props(10+2*(k1-1))
    enddo
