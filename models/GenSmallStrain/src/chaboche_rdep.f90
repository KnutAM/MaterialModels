! Material parameter definition for the Chaboche model, nprops = 5+2*nback
! Input:    props = E, v, Y0, Hiso, invYiso, tstar, nexp, Hk1, invYk1, Hk2, ...
! Internal: mpar  = G, K, Y0, Hiso, invYiso, tstar, nexp, {delta}, Hk1, invYk1, {mk1}, Hk2, ...
...
subroutine checkinput(nprops, nstatv, props, mpar)
    ...
    if (error) then
        write(*,*) 'Invalid input to the chaboche rate dependent model (umat), it should be'
        write(*,*) 'nprops = 7+2*nback and'
    ...
    mpar(3:7) = props(3:7)
    do k1 = 1,nback
        mpar( 9+3*(k1-1)) = props(8+2*(k1-1))
        mpar(10+3*(k1-1)) = props(9+2*(k1-1))
    enddo
