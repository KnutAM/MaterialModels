...
!include 'Hutchinson1976exp_Lindfeldt_AF_bcc.f90'   ! Particular model functions generated by AceGen (example)
...
! change name to umat if used directly without taylor_model and uncomment the !DEC$... if used directly with matmodfit
SUBROUTINE umat_local_orientation(stress,statev,ddsdde,sse,spd,scd, &
...
double precision, allocatable   :: jac_inv(:,:), r0(:), x0(:), yielding(:), TauRedTr(:), param(:), statevNew(:)
...
! Iteration settings
newton_tolerance = 1.d-10  
max_iter = 20

! Check input
call checkinput(nprops, nstatv) ! Assigns nvar and nslip
allocate(jac_inv(nvar, nvar))
allocate(r0(nvar), TauRedTr(nslip), yielding(nslip), statevNew(nstatv))

! Check elastic response
Fnew = m3x3_2_v9(dfgrd1)
call elastic(props,statev,Fnew,stressi,ddsddei,yielding,TauRedTr)


if (any(yielding>0)) then ! Solve for plasticity
    ...
    call setup(x0, param, Fnew, TauRedTr, dtime, statev)

    !Solve local problem using newton raphson iterations and analytical jacobian
    call newton_raphson_ana(residualfun, anajac, x0, jac_inv, lconv, info, props, statev, param, printres=0)
    !call newton_raphson_num(residualfun, x0, jac_inv, lconv, info, props, statev, param, printres=1)
    
    !Calculate ddsdde, statev and stress from solution of local problem
    call plastic_output(x0,props,statev,Fnew,jac_inv,TauRedTr,dtime,stressi,ddsddei,statevNew,spd)
